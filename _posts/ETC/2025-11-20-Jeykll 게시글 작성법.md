...
slug: "jekyll post writing guide"
author: bienew22
date: 2025-11-20 13:13:00 +0900
last_modified_at: 2026-04-10 17:10:00 +0900
categories: [etc, ]
tags: []
---
...
3. `Front Matter > categories` 직접 입력X
  * `_posts/a/b/post.md` 이면 `a`는 최상위 카테고리 `b`는 하위 카테고리로 자동으로 변환
...
#### 미리보기 사진 추가된 링크
- **관련 파일**
  - `logic file` : `_plugins/link-card.rb`
  - `style file` : `_sass/custom/_link_card.scss`
...
- **관련 파일**
  - `logic file` : 
    - `_includes/my/notion-toggle.html`
  - `style file` : 
    - `_sass/custom/_notion-toggle.scss`

- **사용 규칙** 

  ```text
  {% raw %}{% capture body %}{% endraw %}
  토글 내용
...
#### 인라인 수학 블럭
- **관련 파일**
  - `logic file` : `_plugins/math.rb`
  - `style file` : `_sass/custom/_math.scss`

- **사용 규칙** 

  ```
  {% raw %}{% math "content" inline=true %}{% endraw %}

  content
  - 내용물로 원하는 값을 넣을 수 있음.
  - 단, 수식내 수식 중첩은 미 지원.
  - ex) {% raw %}{{2^10}} in python is `2 ** 10` {% endraw %}

  현재 지원하는 수식
  - 제곱: {% raw %}{{BASE^EXP}}{% endraw %}
