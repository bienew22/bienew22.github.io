...
이를 위해 오일러의 체는 현재 수 `x`와 지금까지 발견한 소수 `p`를 이용하여 `x * p`를 생성하여 합성수임을 판정합니다.
...
만약 `x`의 최소 소인수 `p`가 보다 큰 소수 `P`를 사용한다고 가정해 보겠습니다.
...
를 생성하여 `t`가 합성수임을 판정합니다. 그런 `p`는 `x`의 최소 소인수이므로 `x`는 다음과 같이 표현할 수 있습니다.
...
가 됩니다. 따라서 `t`는 이후 `k * P`를 처리하는 과정에서 다시 생성될 수 있습니다. 결국 최소 소인수보다 큰 수를 사용하면 동일한 합성수를 여러 번 방문하게 됩니다.
